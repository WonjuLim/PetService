...
webServer.post("/login.json", function (request, response) {
  ...
  let queryParam = [request.body["user_id"], request.body["password"]];
  ...
  queryResult
      ...
      if (result.rows[0].count > 0) {
        ...
        try {
          request.session.save(function () {
            response.json({ isSuccess: true, message: "로그인성공" });
          });
        } catch (e) {
    ...
    .catch(function (error) {
      console.log("login.json error :", error);
      response.json(error);
    });
});
...
webServer.post("/careRequestInsert.json", function (request, response) {
  ...
  let queryParam = [
    ...
    request.body["care_provide_user_id"]
  ];
...
webServer.get("/careRequestSelectOne.json", function (request, response) {
  ...
  queryResult
    .then(function (result) {
      console.log(
        "/careRequestSelectOne.json queryResult result.rows",
        result.rows
      );
      response.json(result.rows);
    })
...
webServer.get("/careRequestDelete.json", function (request, response) {
  ...
  queryResult
    .then(function (result) {
      console.log(
        "/careRequestDelete.json queryResult result.rows",
        result.rows
      );
      response.json(result.rows);
    })
